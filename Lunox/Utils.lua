...
function Utils:getPath(instance)
	...
	while pathParts[#pathParts].Parent ~= game do
		pathParts[#pathParts+1] = pathParts[#pathParts].Parent
	end
	for i = #pathParts, 1, -1 do
		local part = pathParts[i].Name:gsub("\n", "\\n"):gsub("\"", "\\\"")
		path = path .. (part:match("[a-zA-Z_][a-zA-Z0-9_]+") == part and "." .. part or ('["%s"]'):format(part))
	end
...
function Utils:fixIndex(index)
    local strIndex = tostring(index):gsub("\"", "\\\"")
