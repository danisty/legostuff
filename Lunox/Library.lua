...
function Library:Window(title, icon, tools, parent)
	...
	iBeganConn = UIS.InputBegan:Connect(function(input)
		...
		if input.UserInputType == Enum.UserInputType.MouseButton1 and utils:find(resize, true) and (not cancelResize or (window.Snapped and (topWindow and topWindow.Snapped))) then
	...
	local function inputChanged(input)
		if input.UserInputType == Enum.UserInputType.MouseMovement and not window.Maximized then
			...
			window._Focused = utils:find(resize, true)
			...
			local canSnap = dragController.Dragging and utils:find(snap, true)
	...
	Library:ContextMenu(TitleBar, Window, nil, {"Minimize", "Maximize", 0, "Close"}, function(option)
...
function Library:TabControl(parent, defaultContent)
	...
	function tabControl:AddTab(name, content, closeCallback)
...
function Library:ContextMenu(handle, parent, offset, options, callback, showOnLeftClick, hideOnFocusLoss)
...
function Library:RecyclerFrame(parent, position, size, adapter)
	...
	local function renderItems()
		if recycler.Adapter ~= nil and not rendering then
			rendering = true
			local renderAgain = recycler.Adapter.RenderAgain
			local onRenderItem = recycler.Adapter.OnRenderItem
			filteredItems = searchKeyword and (searchKeyword==lastSearchKeyword and filteredItems or recycler.Adapter:OnSearch(searchKeyword)) or recycler.Adapter.Data

			Recycler.CanvasSize = UDim2.new(0, 0, 0, #filteredItems*fixedSize - (adapter.Padding or 0))
			local position = math.min(Recycler.CanvasPosition.Y, Recycler.CanvasSize.Y.Offset - Recycler.AbsoluteSize.Y)

			if fixedSize then
				local topPosition = math.max(math.floor(position/fixedSize), 1)
				local endPosition = math.min(topPosition + math.floor(Recycler.AbsoluteSize.Y/fixedSize) + 4, #filteredItems)

				for i,_ in pairs(renderedItems) do
	...
	function recycler:Search(keyword)
		...
		searchKeyword = keyword and keyword:lower() or nil
		renderedItems = {}
		renderItems()
	end

	function recycler:NotifyDataChange(data)
		for i,v in pairs(visibleItems) do
			visibleItems[i].Parent = nil
			visibleItems[i] = nil
		end
	...
	Recycler.Changed:Connect(function(p)
		if (p == "CanvasPosition" or p == "AbsoluteSize") and recycler.Adapter ~= nil then
			local position = math.floor(Recycler.CanvasPosition.Y)
			if (position ~= lastPosition and math.abs(lastPosition - position) > recycler.Adapter.FixedSize) or lastPosition == 0 or p == "AbsoluteSize" then
				renderItems()
